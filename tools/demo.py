...
import torch
torch.set_printoptions(sci_mode=False, precision=6) #讨厌非人类的科学计数法显示
# torch.set_printoptions(precision=1, threshold=float('inf'))
# 恢复到默认显示设置
# torch.set_printoptions(threshold=1000)
# torch.set_printoptions(sci_mode=False,precision=1, threshold=float('inf'))

from yolox.data.data_augment import ValTransform
from yolox.data.datasets import COCO_CLASSES, GTSRB_CLASSES, VisDrone_CLASSES, SkyFusion_CLASSES
from yolox.exp import get_exp
from yolox.utils import fuse_model, get_model_info, postprocess, vis
from gnn_postprocess import *
...
class Predictor(object):
    def __init__(
        ...
        self.gnn_postprocess = GNN_Postprocess(exp.input_size)
        self.model = model
    ...
    def inference(self, img):
        ...
        with torch.no_grad():
            ...
            outputs_decoder2 = outputs.clone()
            ...
            ###边关系传递后处理部分
            outputs2 = ourpostprocess1(
                outputs_decoder2, self.num_classes, self.confthre
                )
            start_time = time.time()
            outputs2 = self.gnn_postprocess.run(img, outputs1, outputs2,self.confthre)
            print("GNN postprocess time: {:.4f}s".format(time.time() - start_time))
